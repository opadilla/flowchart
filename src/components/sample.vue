<template>
  <div id="sample">
    ...
    <p>
      The GoJS Diagram is held in a "diagram" component, and only exposes a
      small fraction of the functionality of <a>Diagram</a>. That includes the
      "modelData" property, to initialize the Diagram to show a model, and the
      "diagram" property, to get access to the <a>Diagram</a>.
    </p>
    <p>
      That component also raises two events: "model-changed" corresponding to
      <a>Model.addChangedListener</a>, and "changed-selection" for the
      "ChangedSelection" <a>DiagramEvent</a>.
    </p>
    <p>
      That component also supports two different ways of modifying the diagram:
      access to the <a>Model</a> via the "model" method so that code can call
      Model methods such as <a>Model.setDataProperty</a>, and an
      "updateDiagramFromData" method. The latter permits data changes without
      calling Model methods to notify Diagrams showing the model, but at the
      cost of not being able to support undo and redo.
    </p>
  </div>
</template>
